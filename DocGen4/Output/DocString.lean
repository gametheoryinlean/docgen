...
1. If the input looks like a source file path (`*.lean` with a `/`), construct the corresponding
   source HTML page URL.
2. Otherwise try to decode it as a `Lean.Name`. If decoding succeeds:
   a. **Local decl match** — present in `name2ModIdx` and non-private: emit a relative decl link.
      Auto-generated eliminators (`Foo.rec`, `Foo.casesOn`, …) prefer their parent type when the
      parent is also available.
   b. **Local module match** — present in `moduleNames`: emit a relative module link.
   c. **Similar local decl** — fuzzy-match by trailing components against the current module.
   d. **External fallback** — emit `externalDeclLink name`, i.e. the Mathlib `find/?pattern=…#doc`
      redirect. Both branches that previously returned `none` now route here, so cross-references
      to dep / Lean-core decls in docstrings become clickable.
3. If the input was not a decodable name and not a `.lean` path, return `none` (the caller can
    ...
    if res.name2ModIdx.contains name && !isPrivateName name then
      ...
      if let .str parent suffix := name then
        if isAutoGeneratedSuffix suffix &&
          ...
          let info? := do
            let idx ← res.name2ModIdx[name]?
            let moduleName ← res.moduleNames[idx.toNat]?
            let module ← res.moduleInfo[moduleName]?
            module.members.iter |> filterDocInfo |>.find? (·.getName == name)
          if info?.isNone then
